...
GATED_HOSTS = ("linkedin.com", "instagram.com", "x.com", "twitter.com", "facebook.com")

GATED_MARKERS = (
    "join linkedin", "sign in to linkedin", "authwall",
    "log in to see", "log in • instagram", "join or log in",
    "you must log in", "log in to facebook", "checking your browser",
    "verify you are human", "enable javascript and cookies",
    "please enable javascript",
)

MIN_REAL_CONTENT_CHARS = 200


@tool
def fetch_url(url: str) -> str:
    """Fetches and reads content from a public webpage, article, GitHub repo, or docs link.
    Cannot read gated/login-walled pages like LinkedIn, Instagram, or X/Twitter profiles —
    those require an authenticated session this tool doesn't have."""
    try:
        headers = {
        ...
        is_gated_host = any(host in final_host for host in GATED_HOSTS)
        looks_blocked = any(marker in text.lower() for marker in GATED_MARKERS)
        too_thin = is_gated_host and len(text) < MIN_REAL_CONTENT_CHARS

        if response.status_code in (401, 403) or looks_blocked or too_thin:
            return (
                "BLOCKED: This page requires login or blocks automated access, "
                "so its real content isn't readable — only a login wall came back. "
                "This applies to LinkedIn, Instagram, and X/Twitter profiles in general. "
                "Public pages like blogs, docs, GitHub repos, and news articles work fine."
            )

        return text[:3000] + "..." if len(text) > 3000 else text
    except Exception as e:
        return f"BLOCKED: Could not fetch URL: {str(e)}"